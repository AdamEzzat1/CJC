//! Output hashing (SHA-256), peak memory, and machine/toolchain metadata.
//! No external crates: the harness has the same zero-dependency policy as CJC.
...
pub fn meta() -> Meta {
    let cpu = cmd("powershell", &["-NoProfile", "-Command", "(Get-CimInstance Win32_Processor | Select-Object -First 1).Name"])
        .filter(|s| !s.is_empty())
    ...
    let rustc = cmd("rustc", &["-V"]).unwrap_or_else(|| "unknown".into());
    let git_sha = cmd("git", &["rev-parse", "--short", "HEAD"]).unwrap_or_else(|| "unknown".into());
    let dirty = cmd("git", &["status", "--porcelain", "--untracked-files=no"])
        .map(|s| !s.is_empty())
        .unwrap_or(true);
    Meta { cpu, threads, os, rustc, git_sha, dirty }
}
...
mod tests {
    ...
    fn peak_rss_is_reported() {
        assert!(peak_rss_bytes().map(|b| b > 0).unwrap_or(true));
    }
}
