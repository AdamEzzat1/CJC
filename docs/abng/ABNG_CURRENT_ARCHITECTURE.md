...
| 0.4 Track A | IN PROGRESS | `cjcl abng …` CLI scaffold + `inspect`/`replay`/`diff`/`explain` shipped (4/5 subcommands; `train` stub remains). JSON snapshot view via `--json` flag on each subcommand. `explain` reads prediction snapshots produced by `abng_predict_snap` (Track A G3.5) and reports lineage + abstain-or-trust verdict. Audit kind `0x1B Routed` shipped (G3.5); `0x1A StatsSnapshot` (G3.7) and `0x1C ProvenanceStamped` (Phase 0.5) reserved. |
