...
fn fuzz_abng_tamper_no_panic() {
    bolero::check!()
        ...
        .for_each(|&(offset, mask): &(u32, u8)| {
            ...
            // (unlikely but possible). Only panic-freedom is the property.
            let r = panic::catch_unwind(panic::AssertUnwindSafe(|| {
                let _ = replay(&blob);
            }));
            assert!(r.is_ok(), "replay panicked on blob with byte {pos} ^= {mask:#04x}");
        });
}
...
fn fuzz_abng_provenance_tamper_no_panic() {
    bolero::check!()
        ...
        .for_each(|&(byte_offset, mask): &(u8, u8)| {
            ...
            let flip = mask | 0x01; // ensure at least one bit flipped
            blob[off] ^= flip;
            let r = panic::catch_unwind(panic::AssertUnwindSafe(|| {
                let _ = replay(&blob);
            }));
            assert!(r.is_ok(), "replay panicked on blob with byte {off} ^= {flip:#04x}");
        });
}
...
fn fuzz_abng_smart_replay_tamper_no_panic() {
    ...
    bolero::check!()
        ...
        .for_each(|&(offset, mask): &(u32, u8)| {
            ...
            assert!(r.is_ok(), "replay panicked on blob with byte {pos} ^= {flip:#04x}");
            let r = panic::catch_unwind(panic::AssertUnwindSafe(|| {
                let _ = sr(&blob);
            }));
            assert!(r.is_ok(), "smart_replay panicked on blob with byte {pos} ^= {flip:#04x}");
        });
}
