...
fn fuzz_categorical_frozen_rejection() {
    bolero::check!().with_type::<Vec<u8>>().for_each(|input: &Vec<u8>| {
        ...
        // Contract (`ByteDictionary::freeze` docs): after freezing, `intern`
        // returns `Err(Frozen)` for *unknown* values; a known value resolves
        // to its existing code. Either way the dictionary never extends.
        for v in probe {
            let in_seed = seed.iter().any(|s| s == v);
            let r = dict.intern(v);
            if in_seed {
                let known = dict.lookup(v).expect("lookup must resolve known value after freeze");
                assert_eq!(r, Ok(known), "frozen intern of known value {v:?} must return its code");
            } else {
                assert_eq!(r, Err(ByteDictError::Frozen), "frozen dict accepted unknown value {v:?}");
            }
        }
