//! Bolero fuzz targets for Polytrace. On Windows these run as proptest; on Linux
//! CI they can be promoted to libfuzzer/AFL coverage-guided fuzzing.
//!
//! Three engine target classes (per `SESHAT_IMPLEMENTATION_PROMPT.md` §Tests):
//!
...
//!
//! (The `.cjcl` builtin-arg fuzz lives in the `horus` crate, which owns the
//! `dispatch_seshat` surface.)

use polytrace::{
    analyze_trace, replay, serialize, FrameKind, OwnershipDomain, ThreadState, Trace,
};
...
fn fuzz_structural_analysis_bounded_and_stable() {
    bolero::check!()
        ...
        .for_each(|data: &Vec<u8>| {
            ...
            let share = polytrace::analyze::pct_milli(
