...
//!
//! - **H₂** (hydrogen molecule): 2-qubit reduced encoding, STO-3G, electronic
//!   energy only (add nuclear repulsion for the total energy).
//! - **LiH** (lithium hydride): 4-qubit frozen-core CAS(2,2), STO-3G, generated
//!   with PySCF + OpenFermion; eigenvalues are total energies.
//!
//! # Determinism
//!
...
///
/// This is the **electronic** Hamiltonian: nuclear repulsion (1/R = 0.713754 Ha)
/// is not included. Its ground-state eigenvalue is −1.851199 Ha, within
/// 1.75×10⁻⁴ Ha of the STO-3G FCI electronic energy (−1.851024 Ha, PySCF 2.14).
/// Adding 1/R gives −1.137445 Ha vs the FCI total −1.137270 Ha.
///
/// Only the ground state is accurate: the other three eigenvalues differ from
/// the corresponding physical H₂ levels by 19–24 mHa (4-decimal coefficients
/// from an unrecorded source). Do not use for excited-state or dynamics
/// accuracy claims. See docs/quantum_simulation_research_stack/VERIFY_FOLLOWUPS.md.
///
...
pub fn h2_hamiltonian() -> FermionicHamiltonian {
    ...
    let g0 = -0.4804; // Electronic constant (nuclear repulsion NOT included)
...
/// Build the 4-qubit LiH molecular Hamiltonian.
///
/// Model: LiH, STO-3G, R = 1.546 Å, RHF orbitals, frozen Li 1s core, active
/// space of 2 electrons in 2 spatial orbitals (CAS(2,2)), Jordan–Wigner mapped.
/// Qubit k is spin orbital k: 0 = HOMO↑, 1 = HOMO↓, 2 = LUMO↑, 3 = LUMO↓.
/// The constant term contains nuclear repulsion + frozen-core energy, so
/// eigenvalues are **total** energies:
///
/// - Hartree–Fock determinant (qubits 0, 1 occupied): −7.8631336887 Ha (= E_RHF)
/// - Ground state in the 2-electron sector (CASCI(2,2)): −7.8633736643 Ha
/// - For reference, full-space STO-3G FCI (12 qubits): −7.8827618487 Ha
///
/// Coefficients are generated by
/// `docs/quantum_simulation_research_stack/verification/gen_lih_hamiltonian.py`
/// (PySCF 2.14.0 + OpenFermion 1.8.1). They replace an earlier hand-written
/// "simplified" LiH whose 2-electron ground state was −10.009 Ha, about 2.1 Ha
/// below the true value.
pub fn lih_hamiltonian() -> FermionicHamiltonian {
    // (Pauli string with qubit 0 first, coefficient in Hartree)
    const LIH_TERMS: [(&str, f64); 27] = [
        ("IIII", -7.5062110704095355),
        ("IIIZ", -0.014474984044152622),
        ("IIZI", -0.014474984044152636),
        ("IIZZ", 0.08460455279681561),
        ("IXIX", -0.0016928412401725078),
        ("IXZX", 0.013612631304650511),
        ("IYIY", -0.0016928412401725078),
        ("IYZY", 0.013612631304650511),
        ("IZII", 0.15820955738269765),
        ("IZIZ", 0.053163104044944495),
        ("IZZI", 0.05629936837351339),
        ("XIXI", 0.011919786223087373),
        ("XXYY", -0.0031362643285688945),
        ("XYYX", 0.0031362643285688945),
        ("XZXI", 0.01361263130465051),
        ("XZXZ", -0.0016928412401725078),
        ("YIYI", 0.011919786223087373),
        ("YXXY", 0.0031362643285688945),
        ("YYXX", -0.0031362643285688945),
        ("YZYI", 0.01361263130465051),
        ("YZYZ", -0.0016928412401725078),
        ("ZIII", 0.15820955738269765),
        ("ZIIZ", 0.05629936837351339),
        ("ZIZI", 0.053163104044944495),
        ("ZXZX", 0.011919786223087371),
        ("ZYZY", 0.011919786223087371),
        ("ZZII", 0.1227668566089021),
    ];

    let mut h = FermionicHamiltonian::new(4);
    for (ops, coeff) in LIH_TERMS {
        let mut t = PauliTerm::identity(4);
        t.coeff = ComplexF64::real(coeff);
        for (q, c) in ops.chars().enumerate() {
            t.ops[q] = match c {
                'X' => Pauli::X,
                'Y' => Pauli::Y,
                'Z' => Pauli::Z,
