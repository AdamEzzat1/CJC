...
pub fn run(args: &[String]) {
    ...
    let max_rel_error = entries.iter()
        .map(|e| e.rel_error)
        .fold(0.0f64, f64::max);
    let f32_epsilon: f64 = 1.1920929e-7; // f32::EPSILON as f64
    let stable = max_rel_error < f32_epsilon;
    ...
    match pa.output {
        ...
        _ => {
            eprintln!("{} Precision analysis for `{}` (seed={})",
            ...
            eprintln!();
            eprintln!("Max relative error: {}", format_sci(max_rel_error));
            let verdict = if stable {
                output::colorize(pa.output, output::BOLD_GREEN,
                    "STABLE (within f32 epsilon bounds)")
            } else {
                output::colorize(pa.output, output::BOLD_YELLOW,
                    "SENSITIVE (exceeds f32 epsilon)")
            };
