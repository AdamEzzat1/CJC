...
// ---------------------------------------------------------------------------
// Trained (fit-from-data) per-pass table — v3 (Option A: per-pass diagnostic
// counts via `apply_pass_with_diagnostics`)
// ---------------------------------------------------------------------------
//
// Generated by `cargo run --release -p cana-train-cost-model` from the
// 69-program corpus in `bench/cana_train_cost_model/programs.rs`.
//
// **Signal: each pass's NATIVE diagnostic count.** Every pass reports its
// own `changes_applied` via the `PassDiagnostics` struct in cjc-mir:
//   * CF + DCE: node-count delta (exact)
//   * SR: count of `try_strength_reduce` rewrites
//   * CSE: count of variable-replacement applications
//   * LICM: count of statements hoisted out of `while` loops
//
// Deterministic, zero-variance, byte-identical across runs/OS/CPU.
// Wall-clock measurement was eliminated entirely.
//
// v1 → v3 RMSE reduction (multiplicative):
//   * CF:   0.182 → 0.045   (4.0×)
//   * DCE:  0.166 → 0.067   (2.5×)
//   * SR:   0.130 → 0.029   (4.5×)
//   * LICM: 0.169 → 0.012   (14.1×)
//   * CSE:  0.137 → 0.011   (12.5×)
//
// Every pass now has confidence ≥ 0.59 (v1 baseline was 0.10-0.32).
