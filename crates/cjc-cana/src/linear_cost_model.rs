...
// ---------------------------------------------------------------------------
// Trained (fit-from-data) per-pass table — v2 (60-program corpus + hybrid
// signal: MIR-node-count for CF/DCE, wall-clock for CSE/SR/LICM)
// ---------------------------------------------------------------------------
//
// Generated by `cargo run --release -p cana-train-cost-model` from the
// 60-program corpus in `bench/cana_train_cost_model/programs.rs`.
//
// Signal selection (see main.rs SIZE_SHRINKING_PASSES vs RUNTIME_ONLY_PASSES):
//   * CF + DCE: MIR-node-count delta. Deterministic, zero-variance. RMSE 0.05-0.10.
//   * SR, CSE, LICM: wall-clock median-of-21. Noisy, run-to-run dependent.
//     RMSE 0.09-0.14, confidence 0.27-0.51.
//
// v1 → v2 RMSE: CF 3.3× cleaner, DCE 1.7×, LICM 1.3×, SR 1.5×, CSE ~flat.
// v1 → v2 confidence: CF 0.10 → 0.65, DCE 0.16 → 0.47, others modest gains.
//
// Wall-clock pass coefficients (SR, CSE, LICM) WILL VARY ~10-30% between
// re-runs due to OS scheduler noise on microsecond-scale benchmarks. The
// deterministic CF and DCE coefficients are reproducible bit-for-bit.
//
// Regenerate after corpus changes; do not hand-edit.
fn trained_pass_coefficients(pass_name: &str) -> Option<PassCoefficients> {
    match pass_name {
        "constant_fold" | "cf" => Some(PassCoefficients {
            w_expr_count: 8.357512e-4,   // train_rmse=0.0549, mean_benefit=0.0160 (mir_count)
            w_loop_depth: -4.841451e-3,
            w_branch_count: -7.276677e-3,
            w_alloc_sites: 0.000000e0,
            base_compile_cost: 0.0500,
            confidence: 0.6506,
        }),
        "cse" | "common_subexpression_elimination" => Some(PassCoefficients {
            w_expr_count: 3.014798e-3,   // train_rmse=0.1403, mean_benefit=0.0730 (wall_clock)
            w_loop_depth: -5.078445e-3,
            w_branch_count: -2.265980e-2,
            w_alloc_sites: 0.000000e0,
            base_compile_cost: 0.0800,
            confidence: 0.2709,
        }),
        "dce" | "dead_code_elimination" => Some(PassCoefficients {
            w_expr_count: 1.457277e-3,   // train_rmse=0.0966, mean_benefit=0.0237 (mir_count)
            w_loop_depth: -9.140510e-3,
            w_branch_count: -1.289059e-2,
            w_alloc_sites: 0.000000e0,
            base_compile_cost: 0.0400,
            confidence: 0.4650,
        }),
        "licm" | "loop_invariant_code_motion" => Some(PassCoefficients {
            w_expr_count: 3.336631e-3,   // train_rmse=0.1349, mean_benefit=0.0749 (wall_clock)
            w_loop_depth: -2.595781e-2,
            w_branch_count: -1.042553e-2,
            w_alloc_sites: 0.000000e0,
            base_compile_cost: 0.0700,
            confidence: 0.2947,
        }),
        "strength_reduce" | "sr" => Some(PassCoefficients {
            w_expr_count: 1.767712e-3,   // train_rmse=0.0868, mean_benefit=0.0435 (wall_clock)
            w_loop_depth: -1.145027e-2,
            w_branch_count: -6.240792e-3,
            w_alloc_sites: 0.000000e0,
