...
- **Status:** Accepted (2026-06-02) — **v0.1 forecaster surface complete.** Sessions 1 (ETS Simple + Holt + backtest), 2 (ARIMA Yule-Walker + Hannan-Rissanen MA), 3 (Kalman local-level + RTS smoother), STL (Cleveland 1990 with simplified loess), and Holt-Winters seasonal ETS (additive + multiplicative) all shipped on `feat/cjc-cronos-scaffolding`. v0.1 ships **grid-search hyperparameter selection** for ETS family (deterministic by construction); gradient-based fitting and CSS MLE refinement deferred to v0.2. **Kalman uses caller-supplied (R, Q)**; MLE via prediction-error decomposition is v0.2. **STL uses simplified loess (locally linear, no robustness weights)**; full Cleveland-Hyndman robust STL is v0.2. **`publish = false` retained** until v0.2.x release-engineering pass
