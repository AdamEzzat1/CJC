...
- [x] **Text drift** — shipped 2026-05-29 (ADR-0040, A5.1+A5.2+A5.3). New `crates/cjc-locke/src/tokenizer.rs` (deterministic zero-dep byte-pair-encoding) + `crates/cjc-locke/src/text_drift.rs` (E9110 vocab KS, E9111 token entropy, E9112 char 3-gram language shift). Tokenizer uses explicit lex tie-break on equal-frequency pairs to guarantee byte-identical training across runs. KS drift reuses the existing `stats::ks_d_statistic` numeric path with token-ID frequency expansion. Char 3-gram detector has no tokenizer dependency — robust to tokenizer choice. cjc-locke `--lib` 335→371 (+36 = 21 tokenizer + 15 text drift); tests/locke 225→242 (+13 integration + 3 proptest + 1 bolero counted as a single test marker — the second bolero target inherits an existing harness); cjc-cli unchanged; ABNG 629 unchanged.
- [ ] **Ontology / taxonomy consistency** — hyphen/underscore variants, common-prefix taxonomy inference, hierarchy fragmentation. *Deferred indefinitely per user — requires careful multi-domain test fixtures across medicine, finance, etc.*
...
- [x] **Governance workflows** — shipped 2026-05-29 (ADR-0039, A3). New `policy.rs` module + `.cjcl-locke.toml` config format + `cjcl locke policy apply` and `cjcl locke gate --policy` CLI integration. Three rule kinds: **Suppression** (drop findings matching `(code, column?)` with recorded `reason`, content-addressed `decision_id` for audit), **Owner annotation** (tag findings with team/code/column filter), **Required-finding policy** (assert findings of code X satisfy `==0 / <= / >= / < / > / ==` threshold). `apply_policy(report, policy)` → `PolicyResult` with deterministic suppression order, attributions grouped by team via BTreeMap, requirements in declaration order. `Policy::fingerprint()` is order-sensitive. `ReportDiff::with_policy` extends the gate; `policy_gate_fails()` composes with the existing severity-threshold check. Extended `toml_min.rs` with `[[array_of_tables]]` syntax. cjc-locke `--lib` 309→335 (+26); tests/locke 211→225 (+14); cjc-cli `--lib` 157→175 (+18 = 12 locke parser + 6 toml_min). ABNG 629 unchanged.
